<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="候诊状态" prop="type">
        <el-select v-model="queryParams.type" placeholder="请选择候诊状态" clearable>
          <el-option
            v-for="dict in dict.type.archives_wait"
            :key="dict.value"
            :label="dict.label"
            :value="dict.value"
          />
    ...
    <el-table v-loading="loading" :data="waitList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="候诊状态" align="center" prop="type">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.archives_wait" :value="scope.row.type"/>
        </template>
      </el-table-column>
...
<script>
import { listWait, getWait, delWait, addWait, updateWait } from "@/api/system/wait";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除候诊室编号为"' + ids + '"的数据项？').then(function() {
        return delWait(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/wait/export', {
        ...this.queryParams
